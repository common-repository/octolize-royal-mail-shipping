...
use OctolizeShippingRoyalMailVendor\WPDesk\Beacon\BeaconGetShouldShowStrategy;
		...
		if ( parent::shouldDisplay() ) {
			if ( isset( $_GET['instance_id'] ) ) { // phpcs:ignore
				...
				try {
					...
					if ( $shipping_method && ( ( $shipping_method instanceof RoyalMailShippingMethod ) ) ) {
			...
			if ( isset( $_GET['section'] ) && sanitize_key( $_GET['section'] ) === 'octolize_royal_mail_shipping' ) { // phpcs:ignore /** @phpstan-ignore-line */
