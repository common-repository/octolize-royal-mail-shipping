...
namespace OctolizeShippingRoyalMailVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\CollectionPoint;
...
use OctolizeShippingRoyalMailVendor\WPDesk\WooCommerceShipping\CollectionPoints\CheckoutHandler;
    ...
    public function handle_rates(\WC_Shipping_Method $method, ErrorLogCatcher $logger, WooCommerceShippingMetaDataBuilder $metadata_builder, WooCommerceShippingBuilder $shipment_builder)
    {
        try {
            ...
            if ($rates_count === count($method->rates)) {
                $logger->info(__('No rates added from collection point rates!', 'octolize-royal-mail-shipping'));
            }
        } catch (CollectionPointNotFoundException $cpe) {
            $logger->error(__('Collection point not found.', 'octolize-royal-mail-shipping'));
        } catch (\Exception $e) {
