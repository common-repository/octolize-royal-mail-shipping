...
class FreeShipping
{
    ...
    {
        $label = $is_applied ? __('Cart value exceeds %s. Free shipping has been applied.', 'octolize-royal-mail-shipping') : __('Cart value doesn\'t exceed %s. Free shipping hasn\'t been applied.', 'octolize-royal-mail-shipping');
        $this->logger->debug(sprintf($label, wc_price($this->get_free_shipping_amount())), array(__('Subtotal', 'octolize-royal-mail-shipping') => $subtotal, __('Free Shipping Amount', 'octolize-royal-mail-shipping') => $this->get_free_shipping_amount()));
    }
    /**
     * @return bool
     */
