...
if (0 === \intval($days_to_arrival_date)) {
    \_e('(Delivery Days: 0 days)', 'octolize-royal-mail-shipping');
    // WPCS: XSS ok.
} else {
    // Translators: time in transit.
    echo \sprintf(\_n('(Delivery Days: %1$d day)', '(Delivery Days: %1$d days)', $days_to_arrival_date, 'octolize-royal-mail-shipping'), $days_to_arrival_date);
    // WPCS: XSS ok.
}
?>
</small></div>
